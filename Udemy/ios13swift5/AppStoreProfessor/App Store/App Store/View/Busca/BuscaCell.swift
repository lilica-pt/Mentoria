    ...
    didSet {
      ...
      if let screenshorUrls = app.screenshotUrls {
        if screenshorUrls.count > 0 {
          self.screenshot1ImageView.sd_setImage(with: URL(string: screenshorUrls[0]), completed: nil)
        }
        if screenshorUrls.count > 1 {
          self.screenshot2ImageView.sd_setImage(with: URL(string: screenshorUrls[1]), completed: nil)
        }
        if screenshorUrls.count > 2 {
          self.screenshot3ImageView.sd_setImage(with: URL(string: screenshorUrls[2]), completed: nil)
        }
      }
    }
  }
